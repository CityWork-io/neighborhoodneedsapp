...
const DynamicRankingSystem = () => {
    ...
    // *** THIS ENTIRE DATA LOADING FUNCTION HAS BEEN REPLACED ***
    useEffect(() => {
      const loadData = () => {
        // Use the Papa Parse library loaded in index.html
        window.Papa.parse("/artifact_test_Sheet17.csv", {
          ...
          complete: (results) => {
            // Filter out any rows that didn't parse correctly or are missing a GEOID
                                  ...
                                  <span className="font-medium">
                                    {(() => {
                                      ...
                                      if (typeof rawValue === "number") {
                                        let val = rawValue;
                                        if (isPercent && val <= 1 && val > 0) val = val * 100;
