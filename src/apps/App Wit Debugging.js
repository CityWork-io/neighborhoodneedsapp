...
const ArcGISWebMap = ({ selectedTract, tractData }) => {
  ...
  useEffect(() => {
    ...
    // Load your existing web map
    const webmap = new WebMap({
      portalItem: {
        id: "b87c81d323a24f6486bfbbb8d323b378",
        portal: {
          url: "https://city-work.maps.arcgis.com"
        }
      }
    });
    ...
    // Add widgets - Note: using HomeWidget instead of Home
    ...
    view.when(() => {
      // Find the census tract layer
      const tractLayer = webmap.layers.find(
        (layer) =>
          layer.title &&
      ...
      if (tractLayer) {
        console.log("Found tract layer:", tractLayer.title);
        mapView.current = { view, tractLayer };
      } else {
        console.log(
          "Available layers:",
          webmap.layers.map((l) => l.title)
        );
        if (webmap.layers.length > 0) {
          mapView.current = { view, tractLayer: webmap.layers.getItemAt(0) };
        }
      }
    });
...
useEffect(() => {
    console.log("Selection effect running, selectedTract:", selectedTract);
    
    if (!mapView.current || !selectedTract) {
      console.log("Missing requirements:", { mapView: !!mapView.current, selectedTract });
      return;
    }
  
    const { view, tractLayer } = mapView.current;
    
    if (!tractLayer) {
      console.log("Tract layer not found");
      return;
    }
    ...
    // First, let's inspect the layer before querying
    view.whenLayerView(tractLayer).then(async (layerView) => {
      try {
        // Log layer info
        console.log("Layer title:", tractLayer.title);
        console.log("Layer loaded:", tractLayer.loaded);
        
        // Wait for layer to load if needed
        await tractLayer.load();
        
        // Now check fields
        console.log("Available fields:", tractLayer.fields?.map(f => f.name) || "No fields");
        
        // Let's first do a simple query to see what's in the layer
        const testQuery = tractLayer.createQuery();
        testQuery.where = "1=1";
        testQuery.returnGeometry = false;
        testQuery.outFields = ["*"];
        testQuery.num = 3;
        
        const testResult = await tractLayer.queryFeatures(testQuery);
        console.log("Sample features from layer:");
        testResult.features.forEach((f, i) => {
          console.log(`Feature ${i}:`, f.attributes);
        });
        
        // Now try to find the right field name
        const possibleFields = ['FIPS', 'GEOID', 'GEOID10', 'GEOID20', 'TRACTCE', 'GEOID_Data'];
        let fieldToUse = null;
        
        if (testResult.features.length > 0) {
          const sampleAttributes = testResult.features[0].attributes;
          for (const field of possibleFields) {
            if (sampleAttributes.hasOwnProperty(field)) {
              fieldToUse = field;
              console.log(`Found matching field: ${field} with value: ${sampleAttributes[field]}`);
              break;
            }
          }
        }
        ...
        if (!fieldToUse) {
          console.error("Could not find a suitable field for querying. Available fields:", Object.keys(testResult.features[0]?.attributes || {}));
          return;
        }
        ...
        console.log("Running query:", query.where);
        
        const result = await tractLayer.queryFeatures(query);
        ...
        if (result.features.length > 0) {
          ...
          console.log("No features matched the selected tract:", selectedTract);
        }
        
      } catch (error) {
        console.error("Error in tract selection:", error);
      }
    });
  }, [selectedTract]);
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      ...
      <div 
      ref={mapDiv} 
      style={{ width: "100%", height: 400, background: "#eee", border: "2px solid red" }}
          ...
          <div className="space-y-2 max-h-[65vh] overflow-y-auto">
            {displayedRankings.map((tract) => (
              <div
                key={tract.GEOID}
                onClick={() => {
                  console.log("Clicked tract GEOID:", tract.GEOID);
                  setSelectedTract(tract.GEOID);
                }}
