...
const ArcGISWebMap = ({ selectedTract, tractData }) => {
  ...
  useEffect(() => {
    if (!mapView.current || !selectedTract) return;
    ...
    // Query and highlight selected tract
    view.whenLayerView(tractLayer).then((layerView) => {
      const query = tractLayer.createQuery();
      query.where = `FIPS = '${selectedTract}' OR {FIPS} = '${selectedTract}' OR GEOID = '${selectedTract}' OR GEOID10 = '${selectedTract}' OR GEOID20 = '${selectedTract}'`;
